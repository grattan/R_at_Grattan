...
Looks first-rate! Now you can export as a full-slide Grattan chart using `grattan_save`:
...
Looks super: you have one observation (row) for each state $\times$ gender group you want to plot, and a value for their average income. Put `state` on the x-axis, `average_income` on the y-axis, and split gender by fill-colour (`fill`).
...
Looks brilliant! Now you can add titles and a caption, and save using `grattan_save`:
