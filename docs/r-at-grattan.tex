...
\NormalTok{         occupation }\OperatorTok{==}\StringTok{ "Professionals"}\NormalTok{,}
         ...
         \OperatorTok{!}\NormalTok{sex }\OperatorTok{==}\StringTok{ "Persons"}\NormalTok{) }
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{sex_label =} \KeywordTok{if_else}\NormalTok{(sex }\OperatorTok{==}\StringTok{ "Females"}\NormalTok{, }
...
\StringTok{  }\KeywordTok{geom_text}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ label_data_short,}
            \KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =}\NormalTok{ sex_label))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{prof_chart }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_text}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ label_data,}
            \KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =} \KeywordTok{c}\NormalTok{(}\StringTok{"Women"}\NormalTok{, }\StringTok{"Men"}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

\includegraphics{Data_visualisation_files/figure-latex/unnamed-chunk-4-1.pdf}

To have more freedom over \emph{where} your labels are placed, you can create a dataset yourself. Add the \texttt{x} and \texttt{y} values for your labels, and the label names.\footnote{We are using the \texttt{tribble} function here to make it a little bit clearer what values apply to which sex. The `normal' way to create a tibble is with the \texttt{tibble} function: \texttt{tibble(x\ =\ c(10,\ 100),\ y\ =\ c(100,\ 10))}, etc.}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{self_label <-}\StringTok{ }\KeywordTok{tribble}\NormalTok{(}
\OperatorTok{~}\NormalTok{sex,      }\OperatorTok{~}\NormalTok{sex_label, }\OperatorTok{~}\NormalTok{workers,   }\OperatorTok{~}\NormalTok{median_income,}
\StringTok{"Females"}\NormalTok{,    }\StringTok{"Women"}\NormalTok{,    }\DecValTok{23000}\NormalTok{,            }\DecValTok{55000}\NormalTok{,}
\StringTok{"Males"}\NormalTok{,        }\StringTok{"Men"}\NormalTok{,    }\DecValTok{23000}\NormalTok{,           }\DecValTok{110000}\NormalTok{)}
...
The \texttt{sa3\_income} dataset will be used for all key examples in this chapter.\footnote{From \href{https://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/6524.0.55.0022011-2016?OpenDocument}{ABS Employee income by occupation and sex, 2010-11 to 2015-16}} It is a long dataset from the ABS that contains the median income and number of workers by Statistical Area 3, occupation and sex between 2010 and 2015.
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\NormalTok{sex }\OperatorTok{==}\StringTok{ "Persons"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{under =}\NormalTok{ average_income }\OperatorTok{<}\StringTok{ }\FloatTok{60e3}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{group_by}\NormalTok{(occupation, sex, under) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{n =} \KeywordTok{sum}\NormalTok{(workers)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{pc =}\NormalTok{ n }\OperatorTok{/}\StringTok{ }\KeywordTok{sum}\NormalTok{(n)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(under }\OperatorTok{==}\StringTok{ }\OtherTok{FALSE}\NormalTok{)}
  

\NormalTok{data }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ occupation,}
             \DataTypeTok{y =}\NormalTok{ pc }\OperatorTok{*}\StringTok{ }\DecValTok{100}\NormalTok{,}
             \DataTypeTok{fill =}\NormalTok{ sex)) }\OperatorTok{+}
...
\includegraphics{Visualisation_cookbook_files/figure-latex/bar2-1.pdf}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Adjust the limits of the axis to accommodate the long labels, meaning we will have to define our own axis-label breaks using the \texttt{seq} function\footnote{\texttt{seq(x1,\ x2,\ y)} will return a vector of numbers between \texttt{x1} and \texttt{x2}, spaced by \texttt{y}. For example: \texttt{seq(0,\ 10,\ 2)} will produce \texttt{0\ \ 2\ \ 4\ \ 6\ \ 8\ \ 10}}:
\item
  Add empty space at the top of the chart to accommodate the long labels:
\end{enumerate}
...
\StringTok{  }\KeywordTok{geom_label_repel}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =}\NormalTok{ sa4_name),}
