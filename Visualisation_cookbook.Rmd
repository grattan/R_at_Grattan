...
For this section, you will use the `sa3_income` dataset in 201

```{r scatter_data, message = FALSE}
income2015 <- sa3_income %>% 
  filter(year == 2015)

head(income2015)
```

To make a scatter plot with `average_income` against `workers`, pass the `income2015` dataset
...
You can join the `sf` objects from `absmapsdata` to your dataset using `left_join`. The variable names might be different -- eg `sa4_name` compared to `sa4_name_2016` -- so use the `by` argument to match them.
